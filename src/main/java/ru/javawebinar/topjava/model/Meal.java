...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.GET_BETWEEN, query = "SELECT m FROM Meal m " +
                "WHERE m.user.id=?1 AND m.dateTime BETWEEN ?2 AND ?3 ORDER BY m.dateTime DESC"),
        @NamedQuery(name = Meal.BY_MEALID, query = "SELECT m FROM Meal m WHERE m.id=?1 AND m.user.id=?2"),
//        @NamedQuery(name = Meal.UPDATE, query = "UPDATE Meal m SET m.dateTime = :datetime, m.calories= :calories," +
//          "m.description=:desc where m.id=:id and m.user.id=:userId")
        @NamedQuery(name = Meal.GET_USER, query = "SELECT m FROM Meal m LEFT JOIN FETCH m.user WHERE m.id=?1 AND m.user.id=?2"),
